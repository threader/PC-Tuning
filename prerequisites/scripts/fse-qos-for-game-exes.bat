	...
	for %%a in ("!application_path!") do (
		if /i "%%~xa"==".exe" (
			...
			if !errorlevel! == 1 (
				...
				Reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows\QoS\!profile_name!" /v "Application Name" /t REG_SZ /d "!application_path!" /f
			...
			if !errorlevel! == 1 (
				Reg.exe add "HKCU\SOFTWARE\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers" /v "!application_path!" /t REG_SZ /d "~ DISABLEDXMAXIMIZEDWINDOWEDMODE DPIUNAWARE" /f
			)
